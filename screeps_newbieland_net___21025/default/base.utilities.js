...
function hasContainerNearSource(room, source) {
 ...
 function pickContainerPosition(room, source, spawn) {
    ...
    return bestPlain || bestSwamp || null;
 }

function autoPlanContainers(room) {
    ...
    if (roomSpawn && sourceCandidates.length) {
        best = _.min(sourceCandidates, function(s) {
    ...
    if (buildResult == 0) { return true; }
    return false;
}

function autoPlanExtensions(room) {
...
function containerAvailableForMiner(room, container) {
    var rm = Memory.rooms[room.name];
    
    // If we have no record of containers, then obviously this container is available
    if (!rm || !rm.containers) return true;
    
    // Get the record for this container
    var rec = rm.containers[container.id];
    
    // If no record, then it's free, otherwise see if the creep still exists
    return !rec || !Game.creeps[rec.miner];
}

module.exports = {
    cleanUpCreepMemory: cleanUpCreepMemory,
    autoPlanExtensions: autoPlanExtensions,
    autoPlanContainers: autoPlanContainers,
    hasContainerNearSource: hasContainerNearSource,
    
    containerAvailableForMiner: containerAvailableForMiner,
};